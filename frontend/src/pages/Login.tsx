...
const roleConfig: Record<
  Role,
  { title: string; subtitle: string; icon: typeof User; redirect: string; placeholder: string }
> = {
  customer: {
    title: "Welcome Back",
    subtitle: "Sign in to your Msitiri account",
    icon: User,
    redirect: "/",
  ...
  dealer: {
    title: "Dealer Portal",
    subtitle: "Sign in to manage your dealership",
    icon: Car,
    redirect: "/dealer",
  ...
  admin: {
    title: "Admin Panel",
    subtitle: "Access the Msitiri admin panel",
    icon: Shield,
    redirect: "/admin",
    placeholder: "admin@msitiri.co.ke",
  },
};
...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post("http://127.0.0.1:5000/api/accounts/login/", {
        email,
        password,
      });
            ...
            <Link to="/" className="inline-flex items-center gap-2 mb-6">
              <div
                className={`flex h-12 w-12 items-center justify-center rounded-xl ${
                  role === "admin" ? "bg-destructive" : "gradient-hero"
                }`}
              >
                <config.icon
                  className={`h-6 w-6 ${
